...
h <- highchart() %>% 
  ...
  hc_add_series(name = "Bolezni obtočil (I00-I99)", 
                data = df1.1$`Bolezni obtocil (I00-I99)`) %>%
  hc_add_series(name = "Neoplazme (C00-D48)", 
                data = df1.1$`Neoplazme (C00-D48)`) %>%
  hc_add_series(name = "Nekatere infekcijske, parazitske bolezni (A00-B99)", 
                data = df1.1$`Nekatere infekcijske in parazitske bolezni (A00-B99)`) %>%
  hc_add_series(name = "Poskodbe, zastrupitve, zunanji vzroki (S00-T98)", 
...
# šumniki DONE
# staticno?
# legenda na desni stran -> na desno in da je v enakem zaporedju kot zadnje leto
# sigre lahko dava stran, lahko so samo na legendi(v sivi barvi)
```
...
ggplot(df, aes(x = leto)) +
  ...
  scale_color_manual(values = c('zenske' = 'orange', 'moski' = 'purple')) +
...
server <- function(input, output) {
  ...
  barve = reactive({
    ...
    if (input$izbrano_leto %in% c("2020")){
      c("#56B4E9","#E69F00","#0072B2", "#000000","#D55E00","#009E73")
    } else {
      c("#56B4E9","#E69F00","#0072B2", "#000000","#D55E00")
    }
    })
...
# uskladitev barvu glede na vzrok
