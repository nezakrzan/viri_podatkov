...
ui <- fluidPage(
  headerPanel(''),
  sidebarPanel(
    selectInput("izbrano_leto", label="Izberite leto", choices = unique(df$leto), selected = 2002),
    selectInput("izbran_spol", label="Izberite spol", choices = c("Moski", "Zenske"), selected = "Moski")
  ...
  output$plot <- renderPlotly(
    plot_ly(marker = list(line = list(color = 'white', width = 1)),) %>%
      add_pie(data = data(), labels = ~vzrok, values = ~stevilo.z,
              name = "Zahodna Slovenija", domain = list(x = c(0, 0.4), y = c(0.4, 1))) %>% 
      add_pie(data = data(), labels = ~vzrok, values = ~stevilo.v,
              name = "Vzhodna Slovenija", domain = list(x = c(0.6, 1), y = c(0.4, 1))) %>% 
      ...
      add_annotations(x=c(0.082, 0.7), y=0.3, text = c("Zahodna Slovenija", "Vzhodna Slovenija"), 
