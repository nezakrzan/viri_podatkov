...
S pomočjo logistične regresije bova na podlagi različnih medicinskih spremenljivk (meritev) poskusila napovedati, ali ima posameznica diabetes. Pri tem bova uporabila različne metode za obravnavo mankajočih vrednosti.
...
Za to metodo sva se odločila, saj za izračun manjkajoče vrednosti uporabi tudi preostale vrednosti in poskusi najti najprimernejšo vrednost za nadomestitev manjkajoče vrednosti.

```{r "logisticna regresija KNN metoda", results="hold"}
dataKNN <- KNNimp(data=data, scale=TRUE)

model_knn <- glm(f, data=dataKNN, family = binomial(link="logit"))

selected_stats = describe(dataKNN) %>% 
  dplyr::select(-kurtosis, -skew, -trimmed, -mad, -range)
kable(selected_stats,
      align = "c", digits = 2,
      caption = "Opisne statistike podatkov.") %>%
...
Najprej si oglejmo tabelo manjkajočih vrednosti. Leva stran predstavlja število spremenljivk z manjkajočimi vrednostmi, desna stran pomeni število enot, ter spodaj imamo število enot z manjkajočimi vrednostmi.
...
coefplot(model_knn, add=TRUE, col="blue", offset=-0.1)
coefplot.default(coefs=mice_summary[-1, "estimate"], sds=mice_summary[-1, "std.error"],
                 add=TRUE, col="green", offset=0.2)
legend("bottomright", c("Originalni", "Listwise", "KNN", "MICE"), lty=1, col=c("black", "red", "blue", "green"))
```
