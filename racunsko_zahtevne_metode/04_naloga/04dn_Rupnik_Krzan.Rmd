...
Pri tej metodi na mesto manjkajočih vrednosti (`NA`) vstavimo srednjo vrednost spremenljivke izračunano na podatkih, ki jih imamo na voljo. Za metodo *vstavljanje srednje vrednosti* sva se odločila, saj pričakujeva da bo slabše delovala, vendar naju zanima koliko se bo zares razlikovala od preostalih vrednosti. Gre sicer za dokaj preprosto metodo, ki ni računsko zahtevna in manjkajoče vrednosti nadomestimo na dokaj lahek način. 
...
Za izračun manjkajočih vrednosti sva uporabila funkcijo `mice()`, kjer sva za vrednosti prametrov izbrala `m` = 40 (pravilo palca pravi da izberemo `m` glede na % manjkajočih vrednosti - imamo `round(odstotek,2)`\% mankajočih enot) in `maxit` = 50 (nekoliko višja vrednost, da vrednosti zagotovo skonvergirajo).
...
df_errorbar = data.frame(koef = rep(colnames(data)[1:(ncol(data)-1)], 5),
                     metoda = rep(c("Originalni", "Listwise", "Odločitvena drevesa",
...
vrstni_red = c("Originalni", "Listwise", "Odločitvena drevesa",
