...
Najprej preverimo kakšne vrednosti koeficientov in pripadajočih 95% intervalov zaupanja dobimo v primeru, da za modeliranje uporabimo originalne (nespremenjene podatke).
...
kable(selected_stats,
      align = "c", digits = 2,
      caption = "Opisne statistike podatkov (prvotni podatki).") %>%
kable_styling( full_width = F, 
                position = "center")
```

Če natančneje pogledamo zgornjo tabelo opisnih statistik lahko opazimo, da se ta razlikuje od tabele opisnih statistik, ki smo jo naredili na začetku. Funkcija `glm()` sama po sebi odstrani vse vrstice, ki vsebujejo vsaj eno `NA` vrednost. To je enako kot bi naredili _listwise deletion_. To lahko opazimi tudi iz stolpca, ki prikazuje število enot, na katerih so izračunane opisne statistike (vse vrednosti so enake $392$).
...
kable(selected_stats,
      align = "c", digits = 2,
      caption = "Opisne statistike podatkov (metoda odločitvena drevesa).") %>%
kable_styling( full_width = F, 
                position = "center")
```


Tokrat lahko vidimo, da smo z izračunom nadomestili manjkajoče vrednosti (število vrednosti v vsakem stolpcu (spremenljivki) je enako 768). Pričakovano so se spremenile vrednosti opisnih spremenljivk, predvsem pa opazimo razliko oz. zmanjšanje standardnega odklona (`sd`).

## Multiple imputacije preko verižnih enačb
...
```{r "priprava podatkov MICE", fig.height=7, fig.width=7}
...
Enotam z manjkajočimi vrednostmi najpogosteje manjkajo vrednosti 1, 2 ali 3 spremenljivke. Prav tako skoraj v vseh primerih je vrednost spremenljivke `Insulin` manjkajoča vrednost (le dve enoti imata manjkajoče vrednosti in ta ni pri spremenljivki `Insulin`).

Za izračun manjkajočih vrednosti sva uporabila funkcijo `mice()`, kjer sva za vrednosti prametrov izbrala `m` = 40 (pravilo palca pravi da izberemo `m` glede na % manjkajočih vrednosti) in `maxit` = 50 (nekoliko višja vrednost, da vrednosti zagotovo skonvergirajo).
...
```{r "konvergenca MICE", fig.height=6, fig.width=6}
plot(data_mice, layout=c(2, 5))
```

Glede na zgornje grafe, bi lahko rekli, da smo izbrali prave vrednosti parametrov (vrednosti pri vseh spremenljivkah se ustalijo).
...
```{r "primerjava graficno"}
par(mar=c(0, 7, 0, 0))
coefplot(model_org, main="Ocene regresijskih koeficientov")
coefplot(model_brez_na, add=TRUE, col="red", offset=-0.2)
coefplot(model_RF, add=TRUE, col="blue", offset=-0.1)
coefplot.default(coefs=mice_summary[-1, "estimate"], sds=mice_summary[-1, "std.error"],
                 add=TRUE, col="green", offset=0.2)
legend("bottomright", c("Originalni", "Listwise", "random forest", "MICE"), lty=1, col=c("black", "red", "blue", "green"))
```
