...
############################### p - vrednost ##################################
# var = (1,1,1)
velikosti = c("100, 200, 1000", "20, 25, 30", "500, 500, 500")
settings = expand.grid(i=1:st_ponovitev, n = velikosti)

if(file.exists("p_vrednost_primerjava.RDS")){
  res<-readRDS("p_vrednost_primerjava.RDS")
} else {
  res<-cbind(settings, pANOVA=NA, pANOVA_2=NA, pMETHOD3=NA)
  for(row in 1:nrow(settings)){
    # dolocitev velikosti skupin in varianc
    velikost = as.numeric(strsplit(as.character(settings$n[row]),",")[[1]])
    n1 = velikost[1]
    n2 = velikost[2]
    n3 = velikost[3]
    sd1 = sqrt(1)
    ...
    x<-c(rnorm(n1, 0, sd1),
         rnorm(n2, 0, sd2),
         rnorm(n3, 0, sd3))
    gr<-factor(rep(1:3, times=c(n1,n2,n3)))
  ...
  # shranjevanje
  saveRDS(object = res, file="p_vrednost_primerjava.RDS")
}
...
settings = expand.grid(i=1:st_ponovitev, n = velikosti, povp = povp)

if(file.exists("moc_enake_primerjava.RDS")){
  res_p<-readRDS("moc_enake_primerjava.RDS")
} else {
  res_p<-cbind(settings, pANOVA=NA, pANOVA_2=NA, pMETHOD3=NA)
  for(row in 1:nrow(settings)){
    # dolocitev velikost skupin, variance, povprecja
  ...
  # shranjevanje
  saveRDS(object = res_p, file="moc_enake_primerjava.RDS")
}
