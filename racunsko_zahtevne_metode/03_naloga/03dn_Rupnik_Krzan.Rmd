...
Pri ponovnem vzorčenju bova uporabila metodo bootstrap, pri kateri bo število bootstrap vzorcev enako $m=1000$. Za vsako kombinacijo faktorjev( _alpha_, _gamma_ in _velikost vzorca_) sva generirala podatke, na katerih sva za vsako kombinacijo faktorjev torej izvedla linearno regresijo in poračunala intervale zaupanja za vse tri koeficiente(`Intercept`, `x1`, `x2`). \
...
# Analiza rezultatov pred transformacijo
...
IZ_skupaj = cbind(IZ_skupaj, rep(c("originalni", "bootstrap"), each=nrow(IZ_org)))
...
```{r "risanje IZ pred transformacijo int", fig.cap="Grafi intervalov zaupanja pred transformacijo int."}
pod_int = resLong %>%
  filter(parameter == "int") 

ggplot(pod_int, aes(x=velikost.vzorca, y=value, col=metoda, group=interaction(metoda, tip))) +
  geom_line() + geom_point(cex=0.7) + facet_grid(alpha~gamma, scales="free")
```

```{r "risanje IZ pred transformacijo x1", fig.cap="Grafi intervalov zaupanja pred transformacijo x1."}
pod_x1 = resLong %>%
  filter(parameter == "x1") 

ggplot(pod_x1, aes(x=velikost.vzorca, y=value, col=metoda, group=interaction(metoda, tip))) +
  geom_line() + geom_point(cex=0.7) + facet_grid(alpha~gamma, scales="free")
```

```{r "risanje IZ pred transformacijo x2", fig.cap="Grafi intervalov zaupanja pred transformacijo x2."}
pod_x2 = resLong %>%
  filter(parameter == "x2") 

ggplot(pod_x2, aes(x=velikost.vzorca, y=value, col=metoda, group=interaction(metoda, tip))) +
  geom_line() + geom_point(cex=0.7) + facet_grid(alpha~gamma, scales="free")
```

# Analiza rezultatov po transformaciji
...
IZ_skupaj_transf = cbind(IZ_skupaj_transf, rep(c("originalni", "bootstrap"), each=nrow(IZ_org)))
...
```{r "risanje IZ po transformaciji int", fig.cap="Grafi intervalov zaupanja pred transformacijo int."}
pod_int_transf = resLong_transf %>%
  filter(parameter == "int") 

ggplot(pod_int_transf, aes(x=velikost.vzorca, y=value, col=metoda, group=interaction(metoda, tip))) +
  geom_line() + geom_point(cex=0.7) + facet_grid(alpha~gamma, scales="free")
```

```{r "risanje IZ po transformaciji x1", fig.cap="Grafi intervalov zaupanja pred transformacijo x1."}
pod_x1_transf = resLong_transf %>%
  filter(parameter == "x1") 

ggplot(pod_x1_transf, aes(x=velikost.vzorca, y=value, col=metoda, group=interaction(metoda, tip))) +
  geom_line() + geom_point(cex=0.7) + facet_grid(alpha~gamma, scales="free")
```

```{r "risanje IZ po transformaciji x2", fig.cap="Grafi intervalov zaupanja pred transformacijo x2."}
pod_x2_transf = resLong_transf %>%
  filter(parameter == "x2") 

ggplot(pod_x2_transf, aes(x=velikost.vzorca, y=value, col=metoda, group=interaction(metoda, tip))) +
  geom_line() + geom_point(cex=0.7) + facet_grid(alpha~gamma, scales="free")
```
