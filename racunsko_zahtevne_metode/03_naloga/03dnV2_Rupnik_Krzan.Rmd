...
ggplot(IZ_skupaj, aes(x=velikost.vzorca, col=metoda, group=metoda)) +
...
pokritost_klasicna = pokritost %>% filter(metoda == "klasicna")
```
...
ggplot(pokritostLong, aes(x = factor(velikost.vzorca), y = value, col=metoda, group=metoda)) +
  ...
  facet_grid(factor(alpha, c("1.2", "1", "0.6"))~metric, 
             labeller = label_parsed, scales = "free") +
  labs(x = "velikost vzorca (n)", y = " ") +
  theme(legend.position = "bottom") +
  theme_minimal()
