...
Analizo rezultatov bova razdelila v več podenot, saj pri simulaciji spreminjava veliko število faktorjev (*število skupin*, *število neinformativnih spremenljivk*, *velikost skupin*, razliko med skupinami (*diff*) in korelacijo (*cor*)). Odločila sva se za delitev glede na vrednost povprečja v skupini (razlike povprečja skupin).

Pričakujeva, da bosta metodi pri večjih vrednostih *diff* nekoliko boljše delovali, saj so skupine med seboj dobro ločene oz. je med njimi manj prekrivanja. V nasprotju pa je s spreminjanjem korelacije med spremenljivkami, saj pričakujeva, da bosta metodi nekoliko slabše delovali, ko bo korelcija visoka.
...
```{r fig.cap="Prikaz ARI vrednosti, ko je razlika povprečja skupin enaka 1."}
...
ggplot(pod_diff1, aes(y = value, x = stevilo.skupin,
  ...
  facet_grid(cor~stevilo.neinformativnih.sprem, scales="free")+
  xlab("stevilo skupin") + 
  ylab("ARI") + 
  theme_minimal() + 
  labs(color = "Metoda:") 
...
```{r fig.cap="Prikaz ARI vrednosti, ko je razlika povprečja skupin enaka 2."}
# iz podatkov izbereva le diff=1
...
Tako kot v predhodnem razdelku je v vseh kombinacijah faktorjev možno videti, da vrednost ARI z naraščanje števila skupin pada. Prav tako vrednost pada, ko število neinformativnih spremenljivk narašča. 

## diff = 4

Graf prikazuje spreminjanje ARI vrednosti v odvisnosti od števila skupin, pri čemer vrstice predstavljajo korelacijo med spremenljivkami, stolpci pa število neinformativnih spremenljivk.

```{r fig.cap="Prikaz ARI vrednosti, ko je razlika povprečja skupin enaka 4."}
# iz podatkov izbereva le diff=1
...
```{r fig.cap="Prikaz ARI vrednosti, ko je razlika povprečja skupin enaka 10."}
# iz podatkov izbereva le diff=1
...
S pomočjo ANOVA testa bova med seboj primerjala metodi za razvrščanja v skupine.
