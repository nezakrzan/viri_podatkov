...
Za lažje razumevanje si poglejmo primer, kjer je število skupin enako $8$, število spremenljivk $12$, ter velikost skupin $100$.\
"Posameznike" želimo razporediti v $8$ skupin, zato bo imela vsaka spremenljivka $8*100$ vrednosti.Ker je število skupin enako $8$ bomo imeli 8 informativnih spremenljivk in 4 neinformativne.\
Vrednosti neinformativnih spremenljivk generiramo iz porazdelitve tako da imajo povprečje 0 (torej na podlagi njih ne moremo "posameznike" razvrtiti v skupine). 
Vrednosti informativnih spremenljivk pa generiramo iz porazdlitve tako da imajo 100 vrednosti s povprečjem enakim `diff`, preostalih $(8-1)*100$ vrednosti pa je generiranih iz porazdlitve s povprečjem 0. Naprimer spremenljivka, ki bo v 2. skupini ima 101.-200. vrednost iz porazdlitve s povprečjem `diff`, vse ostale vrednosti pa iz porazdelitve s povprečjem 0.
...
<!--Tom14: ok tukej pač nevem kaj mu ni jasno ker pač ok stevilo skupin sva izbrala kot sva izbrala...mislm lahko spremeniva iz 10 na kej večjega ampak nevem kaj je tle tok bistvenega
To s številom neinformativnih spremenljivk je men cist jasno pac st.sprem-st.skupin...pac isto imava kot smo na vajah delal tko da nevem potem je on slabo naredil..
Za to bivariatno multivariatno pa tud ne razumem oz navem kaj bi se mogla naredit

Mogoce za to dodajanje nepomembnih spremenljivk lahko dopiseva:
"Število pomembnih spremenljivk je enako številu skupin. Vsaka spremenljivka določa eno skupino. Preostale spremenljvke (stevilo spremenljivk - število skupin) pa so nepomembne."
...
V naslednjih dveh primerih je število vseh spremenljivk enako 12, od tega so 4 pomembne (število skupin je enako 4). Torej razlike so prisotne le glede na te spremenljivke (ena od skupin odstopa od preostalih 3). Zaradi preglednosti so grafi, ki prikazujejo odvisnost od preostalih 8 spremenljivk izpuščeni. 
...
Sprva si oglejmo grafe pri ekstremnih vrednostih, torej `diff`=1 in `diff`=10

<!--
```{r diff, fig.cap="Prikaz ARI vrednosti razdeljen glede na velikos in število skupin.", fig.height=4, fig.width=8}
...
```{r diff 1, fig.cap="Prikaz ARI vrednosti razdeljen glede na velikost in število skupin (diff = 1).", fig.height=4, fig.width=8}
df_diff1 = resAggFac_diff %>% filter(diff %in% c(1))

ggplot(df_diff1, aes(y = ari, x = stevilo.spremenljivk,
...
```{r diff 10, fig.cap="Prikaz ARI vrednosti razdeljen glede na velikost in število skupin (diff = 10).", fig.height=4, fig.width=8}
df_diff10 = resAggFac_diff %>% filter(diff %in% c(10))

ggplot(df_diff10, aes(y = ari, x = stevilo.spremenljivk,
                      col=method, group=method)) + 
  geom_point() + geom_line() +
  facet_grid(stevilo.skupin ~ velikost.skupin, scales="free")+
  xlab("stevilo spremenljivk") + 
...
<!-- Tom14: tukej nevem kaj bi potem rekel a da za vsak diff posebi analizirava ampak bojo komentarji skor enaki...ker sej pol naprej obravnavava spet diff pac v anovi pa lin. mes. modeli
-->
...
# k-means anova
aov = aov(value ~ stevilo.spremenljivk*velikost.skupin*stevilo.skupin*diff*method,
                 data = resLongF)
anova(aov) 
```
