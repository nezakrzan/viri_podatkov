...
kable(data.frame(class = c(1,2,3), round(dm, digits=3)), caption="Povprečja po skupinah",
  ...
  kable_styling(full_width=F, latex_options="hold_position")
```
<!--Neza: tole nis zih kako tocno interpretiras...
Tom: jst bi tukej reku da prva funkcija dobro locuje med sorto vina (to se tudi na spodnji slikci dobro vidi), medtem ko druga dobro locuje med 2 in ostalima dvema sortama (ostali dve imata skor enako vrednost)(to se tudi na spodnji slikci dobro vidi)...sicer mi je mal cudno k sm mislu da morjo bit vrednosti med [-1,1] ampak ne najdem kje bi blo kej narobe tko da izgleda je OK-->
...
Preverimo še kakšne vrednosti bi dobili v primeru uporabe CV.
...
Naprej si oglejmo prikaz enot glede na pravo pripadnost sorti vina.
...
Nato pa še pripadnost posamezni skupini glede na klasifikacijo po LDA.
...
Že iz prvega grafa se vidi, da so sorte vina med seboj dobro ločene oz. se med seboj ne prekrivajo. Zato ni presenečenje, da je taka situacija tudi v primeru klasifikacije po LDA. Grafa sta med seboj identična zato lahko zaključimo, da diskriminantna analiza odlično ločujem med skupinami.