...
Na spodnjem grafu si poglejmo povprečne vrednosti spremenljivk za posamezno skupino.
...
y <- t(agr[, -1])
matplot(x=seq_along(ime), y=y, type="o", pch=16, ylim=c(-2, 2), xlab="",
        xaxt="n", ylab="Povprecje (standardizirane)")
axis(side=1, at=seq_along(ime), labels=ime, las=2)
legend("bottomleft", legend=sk.ime, col=1:nsk, pch=16)
abline(h=0, v=1.5)
```
...
Na spodnjem izpisu, si oglejmo število enot v posamezni skupini.

```{r}
# dodamo skupine v podatke
data$cluster <- sk
data$skupina <- factor(x=sk, labels=sk.ime)
...
agr.v1 <- aggregate(v1 ~ skupina, data=data, FUN=mean)
agr.v2 <- aggregate(v2 ~ skupina, data=data, FUN=mean)
cbind(agr.v1, agr.v2)
```
...
plot(x=x, y=y, xlab="dolzina", ylab="mere", asp=1, col=data$skupina, las=1)
legend("bottomright", legend=sk.ime, col=1:nsk, pch=16)
...
( tabela <- 100*proportions(x=table(data$tip, data$skupina), margin=2) )
...
Skupina 2 ima skoraj enakomerno zastopanost med pravimi in ponarejenimi bankovci. Sicer je pravih bankovcev nekoliko več zato bi lahko zaključili, da je skupina 2 šibko povezana s tipom bankovca. \
V skupini 3 pa so skoraj vsi bankovci pravi. Ta skupina je pozitivno povezana s tipom bankovca.


```{r}
# hi kvadrat test
# kontingenzna tabela
tabela <- table(data$tip, data$skupina)
chisq.test(x=tabela, correct=F)
```

Tudi statistični test je z p-vrednostjo $p<0.001$ pokazal, da je statistično značilna povezanost med skupino in tipom bankovca. 
...
Povezanost (0.85) med skupinami in tipom bankovca je zelo močna.

### Povezanost skupin z diagonalo

```{r fig.dim=c(4,4)}
boxplot(diagonala~skupina,data=data)
mx <- aggregate(diagonala ~ skupina, data=data, FUN=mean)[, -1]
abline(h=mean(data$diagonala), col="red")
points(x=mx, col="blue", pch="X")
```

Skupina 1 vsebuje (z izjemo enega) bankovce, ki imajo podpovprečno vrednost diagonale. Povezanost skupine 1 in diagonale je negativna.\
Povprečje vrednosti diagonal bankovcev v skupine 2 se ujema s povprecjem celotnega vzorca. Vendar pa so v njej vsebovani bankovci, ki imajo dosti podpovprečno ali nadpovprečno vrednost diagonale.\
Bankovci v skupini 3 imajo vsi nadpovprečno vrednost diagonale. Povezanost te skupine in vrednosti diagonale je pozitivna.

```{r}
# test povezanosti (vec kategoriji = anova)
# predpostavljamo neenake variance na populaciji
oneway.test(diagonala ~ skupina, data=data, var.equal=F)
```
