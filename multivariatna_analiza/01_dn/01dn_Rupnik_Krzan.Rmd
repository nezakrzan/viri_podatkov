...
v1 <- c('dolzina', 'diagonala')
v2 <- c('levi.rob','desni.rob')
v3 <- c('spodnji.rob', 'zgornji.rob')
...
data$v3 <- rowMeans(x=data[v3])

# opisne statistike
opisne <- as.data.frame(describe(data[c("v1", "v2", "v3")]))
rownames(opisne) <- c("Sklop 1", "Sklop 2", "Sklop3")
round(t(opisne), 2)

# graficni prikaz
oldpar <- par(las=1, mfrow=c(1, 3))
hist(data$v1 , main="Porazdelitev za Sklop 1", xlab="strinjanje", ylab="frekvenca")
hist(data$v2 , main="Porazdelitev za Sklop 2", xlab="strinjanje", ylab="frekvenca")
hist(data$v3 , main="Porazdelitev za Sklop 3", xlab="strinjanje", ylab="frekvenca")
par(oldpar)
...
To je bolje razvidno iz spodnjega grafa
...
matplot(x=seq_along(ime), y=t(agr[, -1]), type="o", pch=16, col=agr[, 1], main="Spol", xlab="", xaxt="n", ylab="")
